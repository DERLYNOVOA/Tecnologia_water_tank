...
public class WaterLevelManager extends SensorLevelManager implements EventListener, IWaterSystemStatus {
    ...
    private final IPumpStrategy strategy;

    public WaterLevelManager(IPump pump, WaterLevelSensor sensor, WaterTank tank, EventHandler handler, RepositoryLog logger, IPumpStrategy strategy) {
        super(20.0f, 80.0f);
        this.pump = pump;
    ...
    public void run() {
        ...
        if (strategy.shouldTurnOff(porcentaje) && pump.getStatus()) {
            pump.turnOff();
            logger.saveLog("[SEGURIDAD] Apagado automático. Nivel: " + porcentaje + "%. Distancia: " + distanciaActual + "cm.");

        } else if (strategy.shouldTurnOn(porcentaje) && !pump.getStatus()) {
            pump.turnOn();
